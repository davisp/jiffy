...
def gen_tests():
	...
	for (min_compat, max_compat) in ERLANG_COMPATIBILITY:
		for (vsn, min_erl, max_erl) in REBAR_MATRIX:
			# Skip any version of rebar that can't be built
			# on the current VM. No version curently has this
			# issue but we're bound to run into it eventually.
...
def run_test(rebar_vsn, build_vm, test_vm):
	...
	cmds = [
		...
		"cd jiffy && {} get-deps compile eunit".format(rebar_path),
		"cd jtapp2 && git clean -ffxd",
		"cd jtapp2 && asdf local erlang {}".format(test_vm),
		"cd jtapp2 && {} get-deps compile".format(rebar_path)
	]
	for cmd in cmds:
